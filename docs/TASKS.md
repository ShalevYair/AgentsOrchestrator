...
**שלוש אבני הדרך:**
- **M1 (סוף P2)** — צ'אט עובד מקצה לקצה עם סוכן אחד. *מוכיח שהצינור חי.*
...
- [ ] **P0-T1 · מונורפו** — pnpm workspaces לפי המבנה ב-[README](../README.md#מבנה-הקוד-המתוכנן).
      *גמור:* `pnpm install` עובד מנקי · `pnpm -r build` עובר · חבילה מייבאת חבילה אחרת.
...
- [ ] **P0-T5 · CI** — GitHub Actions: typecheck + lint + test + build על כל PR.
      *גמור:* CI ירוק · נכשל אמיתית כשמכניסים באג.
...
> **הגדרת גמור לשלב:** `pnpm install && pnpm -r build && pnpm test` עובר על מכונה נקייה. CI ירוק.
...
- [ ] **P1-T3 · אחסון מפתח** — `keytar`, ובנפילה קובץ AES-GCM עם מפתח נגזר-מכונה.
      *גמור:* המפתח לא ב-`localStorage`, לא בלוגים, לא בייצוא · החלפה ומחיקה עובדות · בדיקה למסלול הנפילה.
...
- [ ] **P2-T2 · התמדה** — SQLite: `threads`, `messages`, `runs`, `events`. מיגרציות.
      *גמור:* הודעות שורדות הפעלה מחדש · מיגרציה רצה אוטומטית.
...
- [ ] **P7-T1 · ארגז חול** — תהליך נפרד · **ללא רשת** · כלא מסלולים · פסק זמן · rlimits · רשימת חבילות.
      *גמור:* **בדיקות חדירה: ניסיון רשת, יציאה מהכלא, לולאה אינסופית, פצצת זיכרון — כולן נחסמות**.
- [ ] **P7-T2 · הרצת Python** — `LocalTool` לפי [`PROTOCOLS.md` §11](PROTOCOLS.md#11-localtool).
      *גמור:* stdlib + `pandas`/`numpy` · פלט חורג **נחתך ומסומן**, לא נעלם בשקט.
...
- [ ] **P7-T7 · Docker (אופציונלי)** — בידוד חזק למי שרוצה.
      *גמור:* נופל חזרה בחן אם Docker לא מותקן.
...
- [ ] **P9-T9 · מצבי קצה** — כל 10 המצבים מ-[`UX.md` §10](UX.md#10-מצבים-שחייבים-טיפול-מפורש).
...
- [ ] **P11-T6 · סקירת אבטחה** — כל [`ARCHITECTURE.md` §11](ARCHITECTURE.md#11-אבטחה-ופרטיות) + חדירה לארגז החול.
      *גמור:* דוח כתוב · כל ממצא סגור או מתועד כמודע.
...
- [ ] **P12-T1 · הפעלה בפקודה אחת** — `npx agents-orchestrator` מרים הכל ופותח דפדפן.
      *גמור:* עובד על מכונה נקייה ב-macOS, Linux ו-Windows.
