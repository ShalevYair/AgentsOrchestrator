...
- [x] **P2-T7 · הגדרות** — מסך מפתח API עם אימות חי ואינדיקציה **היכן נשמר**.
      ...
      **באג אמיתי שדווח ע"י משתמש ותוקן:** שמירת מפתח דרך ההגדרות אימתה מול Gemini בהצלחה וכתבה ל-
      `KeyStore`, אבל `ctx.provider` נבחר **פעם אחת** בעליית ה-runtime (`selectProvider`, לפי `GEMINI_API_KEY`
      בסביבה בלבד) ומעולם לא התעדכן אחר כך — משתמש שהזין מפתח תקין ותקף המשיך לקבל את תשובת ה-mock
      ("set GEMINI_API_KEY...") בלי שום דרך לצאת מהמצב הזה חוץ מהפעלה מחדש עם משתנה סביבה מוגדר (שה-UI
      לא מציע). תוקן: `POST /api/keys`/`DELETE /api/keys` (`routes/keys.ts`) מחליפים עכשיו את
      `ctx.provider`/`providerKind`/`model` באופן מיידי דרך `buildProviderFor` החדש ב-`select-provider.ts`
      — אין restart, התשובה הבאה בצ'אט כבר משתמשת בספק הנכון. `startRuntime` גם קורא ל-`keyStore.get()`
      בעליה (`storedApiKey`) כך שמפתח שנשמר בסשן קודם מזוהה גם בהפעלה הבאה בלי משתנה סביבה בכלל. עדיפות:
      `GEMINI_API_KEY` בסביבה גובר על מפתח שמור, ומפתח שנכשל באימות החי לעולם לא מחליף ספק. 10 בדיקות
      חדשות (`select-provider.test.ts`) + בדיקת hot-swap קצה-לקצה חדשה ב-`server.test.ts` (ועוד אישור
      שמפתח שנדחה לא מחליף ספק, בבדיקה הקיימת).
...
- [x] **P12-T1 · הפעלה בפקודה אחת** 🪟 — `npx agents-orchestrator` מרים הכל ופותח דפדפן.
      ...
      *כפי שמומש:* חבילה חדשה `apps/cli` (`agents-orchestrator`) — `src/cli.ts` וכל `@ao/*` שהיא
      תלויה בו (`shared`/`platform`/`providers`/`core`/`tools`/`runtime`) מאוגדים ע"י esbuild ל-`dist/cli.js`
      **יחיד**, כי אף חבילת `@ao/*` לא מפורסמת ל-npm (כולן `private`) — זו הדרך היחידה שהתקנה אמיתית
      מקבלת קוד עובד. `scripts/build.mjs` **גוזר** את `dependencies` האמיתיים (npm רגילים, לא `@ao/*`)
      מתוך package.json של כל חבילה בשרשרת ורושם אותם ב-`apps/cli/package.json` — כך שהם לעולם לא
      יסטו ידנית מהמאוגד בפועל (ולא רק מוצהר) בבנייה; קונפליקט גרסאות בין שתי חבילות על אותה תלות
      נכשל בבנייה, לא נבחר שרירותית. **`devDependencies` (`workspace:*`) מוסרים בזמן `npm pack`/`publish`**
      (`prepack.mjs`/`postpack.mjs` — גיבוי-מחיקה-שחזור סביב package.json) כי `workspace:` הוא לא
      פרוטוקול שאף `npm install` אמיתי מבין. **באג אמיתי שנתפס ותוקן**: איגוד esbuild לקובץ בודד מקריס
      את `import.meta.url` של **כל** המודולים המקוריים לכתובת ה-bundle היחיד — משמעות בפועל: תבנית ה-
      "האם אני נקודת הכניסה?" (`import.meta.url === pathToFileURL(process.argv[1])`) שהייתה ב-
      `apps/runtime/src/index.ts` (מ-P2) ירתה **פעמיים** בתוך ה-bundle של ה-CLI — שרת אחד עם ברירות
      המחדל הנכונות משלי, ושרת שני-בטעות עם ברירות מחדל שגויות (פורט 8787 קשיח, `agentsDir` של הריפו
      במקום הגרסה המצורפת) — נתפס ע"י בדיקה אמיתית (לא מוקית) שמריצה את ה-bundle ובודקת שרק שרת אחד
      עולה. התיקון: `index.ts` הפך לייצוא-בלבד (אפס תופעות לוואי), ונוסף `bin.ts` נפרד כנקודת הכניסה
      האמיתית של `apps/runtime` עצמו (`start`/`dev` עודכנו). **אומת אמפירית בשלושה מעגלים, לא רק
      תיאורטית**: (1) `apps/runtime/src/start.test.ts` — `startRuntime` אמיתי, `listen()` אמיתי,
      `fetch()` אמיתי מעל loopback, כולל בדיקה ש**פורט תפוס נופל בחן לפורט פנוי** (לא רק שהקוד "אמור"
      לעשות זאת) ובדיקה ש-`staticDir` מוגש בלי לחסום את ה-API; (2) `npm pack` אמיתי → `npm install`
      אמיתי (מול registry אמיתי) בתוך תיקייה **מבודדת לגמרי מהמונורפו** (`/tmp`, לא ב-workspace) →
      הרצת ה-binary המותקן → `curl` אמיתי ל-`/api/health` ול-`/` — זה מה שבאמת מוכיח ש"`npx
      agents-orchestrator`" יעבוד, לא רק שה-bundle "נראה תקין"; (3) `apps/cli/scripts/smoke-test.mjs`
      (סקריפט Node טהור, בלי הנחת shell — עובד זהה תחת bash/PowerShell) מריץ בפועל תהליך-בן עם
      ה-bundle, ממתין לאיתות מוכנות אמיתי (לא `sleep` קבוע), ומאמת תשובת HTTP אמיתית — זה מה שרץ ב-CI
      (`.github/workflows/ci.yml`, מיד אחרי `Build`, בשלוש הפלטפורמות). ⚠️ **מה שכן לא אומת בפועל**:
      פתיחת דפדפן על Windows/macOS אמיתיים (`openBrowser` נבדק ביחידה עם `spawnFn` מוזרק — הפקודות
      עצמן [`cmd /c start ""`, `open`, `xdg-open`] מתועדות ומוצדקות אך לא הופעלו על מכונת Windows/macOS
      אמיתית בסביבה הזו), וכנ"ל ריצה מ-PowerShell/CMD ומכונן שאינו `C:` — אלה ייבדקו לראשונה בפועל
      כש-CI ירוץ על `windows-latest`. בחירת הפורט הפנוי **כן** אומתה אמפירית (start.test.ts, לעיל),
      כולל על התנגשות אמיתית בין שני שרתים.
...
- [x] **P12-T3 · תיעוד משתמש** — התקנה, מדריך ראשון, מדריך תקציב, פתרון תקלות.
      *גמור:* משתמש חדש מסיים משימה אמיתית בלי לשאול.
      *כפי שמומש:* `docs/GETTING_STARTED.md` — ארבעה חלקים בדיוק כמו שהמשימה מפרטת. כל עובדה בו
      מאומתת מול הקוד/ה-i18n האמיתיים, לא מומצאת: טקסט כפתורי ה-onboarding מצוטט מדויק מ-
      `apps/web/src/i18n/locales/he.json` (לא ניסוח משוער), טבלת פתרון-התקלות מכסה בדיוק את מה
      שנבנה בפועל ב-[P12-T1](#p12)/[P12-T2](#p12) — נפילת פורט לפורט פנוי, הודעת Python/Docker
      הספציפית לפלטפורמה, הודעת מפתח שגוי מ-P1-T4 — ולא תרחישים שלא אומתו (למשל: לא נטען שיש בעיה
      עם Windows SmartScreen על `npx` — זה מריץ `node.exe` שכבר מותקן ונאמן, לא בינארי לא-חתום
      חדש, ואין לכך ראיה בקוד הזה). מדריך התקציב מתמצת בכוונה את סולם ההידרדרות בן-8-הרמות מ-
      [`BUDGET.md` §5](BUDGET.md#5-סולם-ההידרדרות) לשלוש אפשרויות מדיניות (`degrade`/`ask`/
      `hard-stop`) כפי שמשתמש קצה באמת רואה אותן ב-`GoalForm` — לא כל הפירוט הטכני.
- [x] **P12-T4 · תיעוד מפתחים** — `CONTRIBUTING.md`, מפת הקוד, איך מוסיפים רכיב.
      *גמור:* מפנה ל-[`EXTENDING.md`](#p10) ולמסמכים כאן.
      *כפי שמומש:* `CONTRIBUTING.md` בשורש המאגר. מפת הקוד עודכנה להיות **נכונה לעכשיו**, לא
      "מתוכננת" — כוללת את `apps/cli`+`apps/evals` שלא היו קיימים כש-README.md נכתב במקור, וקובעת
      במפורש את חוק השכבות שנשמר בפועל לאורך P0–P12 (`core`/`platform`/`tools` תלויים רק ב-`shared`,
      `apps/runtime` הוא ההרכבה היחידה). §"איך מוסיפים רכיב" מצביע ל-`EXTENDING.md` ומוסיף אזהרה
      אחת שלא הייתה שם בפירוש: לא כל 11 סוגי הסוכנים מ-ARCHITECTURE.md §4 ניתנים-להרחבה בתיקייה —
      רק 6 (פער תיעודי כן מתועד ב-[P10-T3](#p10), אבל מי שקורא רק EXTENDING.md לא בהכרח מגיע לשם).
      README.md עצמו עודכן במקביל (לא נשאר כמסמך נפרד לא-מסונכרן): הוסר הפתיח שטען "אין קוד עדיין"
      (שקרי מאז P0), נוסף "התקנה מהירה" עם `npx agents-orchestrator`, וטבלת הסטטוס בתחתית עודכנה
      מ-"טרם התחיל" ל-מצב האמיתי (P0–P11 גמורים, P12 בתהליך).
...
- [x] **P12-T7 · טלמטריה** — **מצטרפים מרצון בלבד**, מקומית, ללא תוכן.
      *גמור:* כבויה כברירת מחדל · מה שנאסף מתועד במלואו.
      *כפי שמומש:* `packages/platform/src/telemetry/` — שלושה סוגי אירועים בלבד
      (`app_started`/`run_completed`/`run_failed`), כל אחד `z.strictObject` משלו כדי ש"ללא תוכן" יהיה
      אכיפה מבנית ולא רק מוסכמה: אין דרך להוסיף שדה טקסט חופשי בלי שהסכמה עצמה תדחה אותו. `telemetryEnabled`
      נוסף ל-`ConfigSchema` הקיים (`packages/platform/src/config/`), `false` כברירת מחדל, עם `preprocess`
      ייעודי ל-`AO_TELEMETRY_ENABLED` **שלא** משתמש ב-`z.coerce.boolean()` — `Boolean("false")` הוא `true`
      ב-JS, אז coerce גולמי היה הופך משתנה-סביבה שנועד לכבות לכזה שמדליק; רק המחרוזת המדויקת `"true"` מדליקה.
      `createTelemetryRecorder` (`@ao/platform`) מחזיר no-op אמיתי כשכבוי — לא נוגע בדיסק בכלל, לא רק "כותב
      לוג ריק" — וכשל כתיבה (דיסק מלא, הרשאות) נתפס ונרשם ללוגר, לעולם לא זורק. מחובר ב-`apps/runtime`:
      `start.ts` יוצר את ה-recorder ורושם `app_started` בעלייה; `routes/threads.ts` עוטף את הקריאה הקיימת
      ל-`runChatTurn` (בלי לגעת בפנים שלה בכלל — קוד יציב ובדוק היטב מ-P2/P9) ורושם `run_completed`/
      `run_failed` לפי התוצאה, עם `errorCode` יציב בלבד (`isAppError`) — לעולם לא הודעת השגיאה הגולמית.
      `GET /api/health` נושא `telemetryEnabled` חדש; `TelemetryStatus.tsx` בהגדרות מציג את המצב בפועל +
      בדיוק מה נאסף, בעברית ואנגלית. **מוצג-בלבד, לא מתג חי**: אין עדיין מנגנון שכותב הגדרות UI חזרה
      ל-config.json (אף הגדרה אחרת היום לא עושה זאת — `LanguageSwitcher` הוא i18next צד-לקוח טהור, לא
      תקדים אמיתי) — בניית "כתיבת קונפיג מהשרת" בשביל טלמטריה בלבד הייתה חורגת מהיקף המשימה; ההפעלה/כיבוי
      דרך config.json/משתנה סביבה, מתועד במלואו ב-`docs/TELEMETRY.md` (חדש) ומקושר מ-Settings/
      GETTING_STARTED.md/CONTRIBUTING.md. **אומת אמפירית, לא רק ביחידה**: `server.test.ts` מריץ שיחת צ'אט
      אמיתית מקצה-לקצה עם recorder אמיתי (לא מוקי) מצביע על תיקייה זמנית אמיתית, קורא את קובץ ה-JSONL
      שנכתב בפועל, ומוודא בפירוש **שהמחרוזת "boom"/"sensitive" מהודעת השגיאה הגולמית לא מופיעה בקובץ בכלל**
      — לא רק שהשדה `errorCode` נכון.
- [ ] **P12-T8 · שחרור** — גרסאות סמנטיות, changelog, פרסום.
      *גמור:* v1.0.0 מתויג ומשוחרר.
      *כפי שמומש (חלקית, בכוונה):* כל **תשתית** השחרור מוכנה — `CHANGELOG.md` (Keep a Changelog,
      מדיניות "כל החבילות עולות יחד"), `scripts/bump-version.mjs` (מעדכן `version` בכל ה-package.json-ים
      במונורפו יחד, **לעולם לא** נוגע ב-`workspace:*`/ב-`dependencies` האמיתיים של `apps/cli` — אומת
      ידנית: bump ל-0.2.0-test על 11 קבצים, `git diff` הראה בדיוק שורת `version` אחת שהשתנתה בכל
      קובץ, שוחזר נקי), ו-`RELEASING.md` עם כל שלבי התהליך (bump → changelog → tag → `npm publish`
      מ-`apps/cli` → GitHub Release). **הפעולה עצמה — תיוג ופרסום v1.0.0 בפועל — לא בוצעה במכוון**:
      זו פעולה גלויה-לעולם ובלתי-הפיכה (git tag ציבורי + פרסום ל-npm) שדורשת גם החלטה עסקית שלא
      קיימת עדיין (איזה רישיון — **אין עדיין קובץ `LICENSE`** בפרויקט, ראו ההערה בתחתית
      `RELEASING.md`) וגם אישור מפורש של בעל הפרויקט, לא רק "להמשיך עם P12". `apps/cli/package.json`
      נשאר `"private": true` בכוונה עד לרגע הפרסום עצמו (מונע פרסום-בטעות דרך `pnpm publish -r`
      רחב באמצע פיתוח רגיל) — `RELEASING.md` §6 מתעד את ההסרה הידנית הנדרשת. אין עדיין אוטומציה
      (GitHub Actions על `push: tags`) — תועד כשיפור עתידי סביר, לא נבנה כחלק מ-P12.
