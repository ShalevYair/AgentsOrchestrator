...
פותח דפדפן על שרת מקומי, בפקודה אחת, בשלוש הפלטפורמות. מדריך מלא (דרישות, הרצה ראשונה, פתרון
תקלות): [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md). רוצים לתרום קוד?
[`CONTRIBUTING.md`](CONTRIBUTING.md).

---

> ## 📍 נקודת הכניסה לפיתוח: **[`docs/TASKS.md`](docs/TASKS.md)**
>
> כל העבודה על הקוד מתחילה ומתנהלת מקובץ המשימות — שם רואים מה כבר בנוי ומה הצעד הבא.
> אל תתחיל לכתוב קוד בלי לקרוא אותו.
...
|---|---|---|
| 👤 [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md) | התקנה, הרצה ראשונה, מדריך תקציב, פתרון תקלות | אתם **משתמשים** באפליקציה, לא מפתחים אותה |
| 🧑‍💻 [`CONTRIBUTING.md`](CONTRIBUTING.md) | סביבת פיתוח, מפת קוד, זרימת PR | לפני ה-PR הראשון שלכם |
| 🎯 **[`docs/TASKS.md`](docs/TASKS.md)** | **תוכנית הביצוע — 13 שלבים, משימות ממוספרות, הגדרת "גמור" לכל אחת** | **תמיד. נקודת הכניסה לפיתוח.** |
...
| 🧩 [`docs/EXTENDING.md`](docs/EXTENDING.md) | הוספת סוג סוכן, מתכון, reducer, כלי — בלי לגעת ב-`packages/core` | כשמוסיפים יכולת חדשה |

## מבנה הקוד

מפה מפורטת יותר (כולל חוק השכבות בין החבילות): [`CONTRIBUTING.md`](CONTRIBUTING.md#מפת-הקוד).

```
apps/
  web/                  React + Vite — ממשק המשתמש
  runtime/              שרת Node מקומי — API, WebSocket, התמדה
  cli/                  אריזת `npx agents-orchestrator` — מאגד את השאר לפקודה אחת
  evals/                מסגרת ההערכה — מריצה את משימות הזהב תחת evals/
packages/
...
|---|---|---|
| — | אפיון ותכנון | ✅ הושלם |
| P0–P11 | יסודות, ספק Gemini, תזמור, ממשק מלא, evals והקשחה | ✅ הושלם |
| P12 | אריזה ו-DX (המדריך הזה, `npx agents-orchestrator`, תיעוד) | 🔄 בתהליך — ראה [`docs/TASKS.md`](docs/TASKS.md#p12) |

**רוצים להשתמש באפליקציה עכשיו?** [`docs/GETTING_STARTED.md`](docs/GETTING_STARTED.md).
**רוצים לתרום קוד?** פתחו את [`docs/TASKS.md`](docs/TASKS.md), קחו את המשימה הראשונה שלא מסומנת,
ועקבו אחרי כללי העבודה שבראש הקובץ.
